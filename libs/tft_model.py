...
class TemporalFusionTransformer(object):
  ...
  def get_tft_embeddings(self, all_inputs):
    ...
    print(known_categorical_inputs)
    print("Sum")
    print(known_regular_inputs + known_categorical_inputs)
  ...
  def build_model(self):
    ...
    with tf.compat.v1.variable_scope(self.name):
      ...
      model.compile(
          loss=quantile_loss, optimizer=adam, metrics=[tf.keras.metrics.MeanSquaredError(), tf.keras.metrics.MeanAbsolutePercentageError(), tf.keras.metrics.MeanAbsoluteError()], sample_weight_mode='temporal')

      self._input_placeholder = all_inputs

    return model

  def fit(self, train_df=None, valid_df=None, init_epochs=0):
    ...
    tensorboard_callback = tf.keras.callbacks.TensorBoard(log_dir=os.path.join(self._temp_folder, 'tensorboard'), histogram_freq=0, write_graph=True,
            write_images=True)
    ...
    self.model.fit(
        ...
        epochs=self.num_epochs+init_epochs,
